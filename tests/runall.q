...
.k4.runall:{[run;res]
  ...
  // Get errors from most recent log files and return output list
...
if[all `rundir`resdir in key args:.Q.opt .z.x;
  output:.k4.runall . hsym each `$first each args[`rundir`resdir];
  show each ("Test results:";results:output 0;"Test failures:";fails:output 1;"Logged errors:";errors:output 2)
  ];
...
checktimebar:{[dict]
    ...
    if[not in[size;key .dataaccess.timebarmap];
        '`$.checkinputs.formatstring["the inputted size of the timebar argument: {size}, is not an appropriate size. Appropraites sizes are: {app}";`size`app!(`min;key .dataaccess.timebarmap)]];

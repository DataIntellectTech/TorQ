...
subscribe:{[tabs;instrs;setschema;replaylog;proc]
	...
	/replaylog:1b; /- ### hardcoded replaylog to true for testing ###
	/df:{(.u.sub\:[x;y];(.u.i;.u.L);(.u `icounts))};			/ pull .u.i .u.L and .u.icounts regardless 
	/df:$[replaylog;{(.u.sub\:[x;y];(.u.i;.u.L))};{(.u.sub\:[x;y];(`;`))}]; / original version
	...
	if[count details;
		...
		(`subtables`tplogdate!(details[0;;0];(first "D" $ -10 sublist string last details 1)^logdate)),{(where 101 = type each x)_x}(`i`icounts)!(details[1;0];details[2])
	}

...
schemacheck:{[tab;colname;types;forkeys;attribute]
  ...
  $[all c:((flip origschema)each key flip origschema)~'(flip checkschema)each key flip checkschema;
    ...
    (0b;"The following columns from the schema of table ",(string tab)," did not match expectation: ",(","sv ("columnname";"types";"foreignkeys";"attribute")[where not c]),"Expected: ",(raze string checkschema[(`c`t`f`a)[where not c]]),"Actual Schema: ",(raze string origschema[(`c`t`f`a)[where not c]]))]
  }

...
if[.access.enabled;
	...
	.dotz.set[`.z.pw;{$[.access.vpw[y;z];x[y;z];0b]}.z.pw];
	/ .z.po - untouched, .z.pw does the checking
	/ .z.pc - untouched, close is always allowed
	if[not .access.openonly;
		.dotz.set[`.z.pg;{$[.access.vpg[y];.access.validsize[;`pg.size;y]x y;.access.invalidpt y]}.z.pg];
		.dotz.set[`.z.ps;{$[.access.vps[y];x y;.access.invalidpt y]}.z.ps];
		.dotz.set[`.z.ws;{$[.access.vws[y];x y;.access.invalidpt y]}.z.ws];
		.dotz.set[`.z.pi;{$[.access.vpi[y];x y;.access.invalidpt y]}.z.pi];
		.dotz.set[`.z.ph;{$[.access.vph[y];x y;.h.hn["403 Forbidden";`txt;"Forbidden"]]}.z.ph];
		.dotz.set[`.z.pp;{$[.access.vpp[y];x y;.h.hn["403 Forbidden";`txt;"Forbidden"]]}.z.pp]]];
\

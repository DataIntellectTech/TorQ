...
execute:{
    handle:first -1?h;
    db:first -1?dbs;
    query:last flip -1?select from queries where proc=db;

    (neg handle)(`.gw.asyncexec; eval query; db); handle[]
    };

\d .

.timer.repeat[.proc.cp[]; 0Wp; 0D00:00:30; (`.query.execute;`); "Execute fake queries"];

...
The patcher process is used to easily apply updates and patches to any TorQ process between full releases of the framework. These patches may be anything from simply updating the value of a variable to redefining functions. Furthermore, these patches are also saved to disk and are reapplied whenever a patched process is restarted.

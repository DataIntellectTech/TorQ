...
The **monitalert.cfg** is only an example of how you can configure your own alerts for monitoring your UNIX system. There are no TorQ specific examples in this file.
...
Alternatively, you can start the monit daemon by executing
```
bash monit.sh init
```
and then check your processes by executing
```
bash monit.sh run summary
```
You can also use the following commands to manage how monit monitors the `hdb1` process.
```
bash monit.sh run stop hdb1
bash monit.sh run unmonitor hdb1
```
